...
export const Payment = () => {
  ...
  const breadcrumbItems = [{ label: 'Payments', url: '' }];

  return (
    <div>
      <DynamicBreadcrumb items={breadcrumbItems} />
      <h2 id="payment-heading" data-cy="PaymentHeading">
        {/* <Translate contentKey="schoolMisApp.payment.home.title">Payments</Translate> */}
        <div className="d-flex justify-content-end">
          <Link to="/payment/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            ...
            &nbsp;
            <Translate contentKey="schoolMisApp.payment.home.createLabel">Create new Payment</Translate>
          </Link>
        </div>
      </h2>
        ...
        {paymentList && paymentList.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="schoolMisApp.payment.id">ID</Translate> <FontAwesomeIcon icon={getSortIconByFieldName('id')} />
                </th>
                <th className="hand" onClick={sort('amount')}>
                  <Translate contentKey="schoolMisApp.payment.amount">Amount</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('amount')} />
                </th>
                <th className="hand" onClick={sort('paymentDate')}>
                  <Translate contentKey="schoolMisApp.payment.paymentDate">Payment Date</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('paymentDate')} />
                </th>
                <th className="hand" onClick={sort('method')}>
                  <Translate contentKey="schoolMisApp.payment.method">Method</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('method')} />
                </th>
                <th>
                  <Translate contentKey="schoolMisApp.payment.student">Student</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th />
              </tr>
            </thead>
            <tbody>
              {paymentList.map((payment, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>
                    <Translate contentKey={`schoolMisApp.PaymentMethod.${payment.method}`} />
                  </td>
                  <td>{payment.student ? <Link to={`/student/${payment.student.id}`}>{payment.student.id}</Link> : ''}</td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="schoolMisApp.payment.home.notFound">No Payments found</Translate>
            </div>
          )
        )}
      </div>
